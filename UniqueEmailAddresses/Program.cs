...
namespace UniqueEmailAddresses
{
    ...
    /// mail sent there will be forwarded to the same address without dots in the local name.For example,
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var result = solution.NumUniqueEmails(input);
    ...
    public class Solution
    {
        public int NumUniqueEmails(string[] emails)
        {
            ...
            {
                var strArr = emails[i].Split('@');
                var localName = strArr[0].Replace(".", "").Split('+')[0];
                hashSet.Add($"{localName}@{strArr[1]}");
            }
